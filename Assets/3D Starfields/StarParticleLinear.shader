	...
	SubShader
	{
		...
		Pass
		{
			...
			fragmentInput vert(vertexInput i) 
			{
				...
				float4 vertexPosCamSpace = mul(UNITY_MATRIX_MV, i.vertex);
				float4 vertexPosCamSpaceWithOffset = vertexPosCamSpace + float4(_ParticleSize, 0, 0.0, 0.0);
				...
				o.offset.w = distance(mul(_Object2World, i.vertex), _WorldSpaceCameraPos);
